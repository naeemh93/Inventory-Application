  ...
  describe '.generate_csv_content' do
    ...
    context 'when there are multiple records' do
      it 'returns a CSV string with all records' do
        data = [
          { name: 'Location A', scanned: true, occupied: 1, expected_barcodes: 2, detected_barcodes: 2, comparison_result: 'Match' },
          { name: 'Location B', scanned: false, occupied: 2, expected_barcodes: 4, detected_barcodes: 3, comparison_result: 'Mismatch' }
        ]
